...
author: "Aidan Teeger"
date: today
description: "A walkthrough of running an A/B test on a newsletter call-to-action, using simulated data to illustrate the core ideas behind frequentist statistical inference."
---
...
print(f"CTA A sign-up rate: {cta_a.mean():.3f}")
print(f"CTA B sign-up rate: {cta_b.mean():.3f}")
...
ax.plot(range(1, n + 1), cumulative_avg, color="#2563eb", linewidth=1.2, label="Running average of θ̂")
...
ax.set_title("Law of Large Numbers: Convergence of θ̂ to θ", fontsize=13)
...
for i in range(n_boot):
    a_resample = np.random.choice(cta_a, size=n, replace=True)
    b_resample = np.random.choice(cta_b, size=n, replace=True)
    boot_estimates[i] = a_resample.mean() - b_resample.mean()
...
print(f"Bootstrapped standard error: {boot_se:.4f}")
...
for idx, sz in enumerate(sample_sizes):
    ...
    for i in range(n_sims):
        ...
        estimates[i] = a_draws.mean() - b_draws.mean()
    all_estimates[sz] = estimates

x_min = min(est.min() for est in all_estimates.values()) - 0.02
x_max = max(est.max() for est in all_estimates.values()) + 0.02

for idx, sz in enumerate(sample_sizes):
    ax = axes[idx]
    ax.hist(all_estimates[sz], bins=30, color="#2563eb", edgecolor="white", alpha=0.8, density=True)
    ax.axvline(x=theta_true, color="#dc2626", linestyle="--", linewidth=1.5, label=f"True θ = {theta_true}")
    ax.set_xlim(x_min, x_max)
    ax.set_title(f"n = {sz}", fontsize=12, fontweight="bold")
    ax.set_xlabel("θ̂", fontsize=10)
    ax.set_ylabel("Density", fontsize=10)
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)

plt.suptitle("Sampling Distribution of θ̂ at Different Sample Sizes", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
plt.show()
```
...
3. This is powerful because it means we know the distribution of our test statistic under the null — and that's exactly what we need to compute a **p-value**.
...
if p_value < 0.05:
    ...
    print(f"evidence that the two CTAs produce different sign-up rates.")
else:
    print(f"\nAt α = 0.05, we fail to reject H₀. We do not have sufficient")
    print(f"evidence to conclude the two CTAs produce different sign-up rates.")
```
...
print(f"  θ̂ = {theta_hat:.4f}, SE = {analytical_se:.4f}, z = {z_stat:.4f}, p = {p_value:.4f}")
print(f"\nFrom the regression:")
print(f"  β̂₁ = {model.params[1]:.4f}, SE = {model.bse[1]:.4f}, t = {model.tvalues[1]:.4f}, p = {model.pvalues[1]:.4f}")
```
...
for _ in range(n_experiments):
    a_full = np.random.binomial(1, 0.20, size=1000)
    b_full = np.random.binomial(1, 0.20, size=1000)
    ...
    for peek_n in peek_points:
        ...
        se = np.sqrt(p_hat_a * (1 - p_hat_a) / peek_n + p_hat_b * (1 - p_hat_b) / peek_n)
        if se > 0:
            z = diff / se
...
print(f"False positive rate with peeking (10 peeks): {fpr:.3f}")
